...
export async function fetchWorkers(): Promise<WorkerProfile[]> {
  try {
    ...
    return data.map((d: any) => ({
      ...
      isIdVerified: Boolean(d.is_id_verified ?? d.verified),
      hasPoliceClearance: Boolean(d.has_police_clearance ?? d.verified),
      hasBarangayClearance: Boolean(d.has_barangay_clearance ?? d.verified),
      certifications: d.certifications || [],
      barangay: d.barangay || "Tagum City",
