    ...
    <div className="flex min-h-screen items-center justify-center bg-background px-4">
      <div className="surface-elevated max-w-md p-8 text-center">
        ...
        {import.meta.env.DEV && error.message ? (
          <pre className="mt-4 max-h-40 overflow-auto rounded-2xl bg-muted p-3 text-left font-mono text-xs text-destructive">
            {error.message}
          </pre>
        ) : null}
        <div className="mt-6 flex items-center justify-center gap-3">
          ...
          <a
            href="/"
            className="inline-flex items-center justify-center rounded-full border border-input bg-background px-4 py-2 text-sm font-semibold text-foreground transition-transform hover:-translate-y-0.5"
          >
            Go home
          </a>
        </div>
